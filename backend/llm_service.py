    ...
    def __init__(self):
        ...
        try:
            ...
            self.chain = ConversationalRetrievalChain.from_llm(
                ...
                combine_docs_chain_kwargs={"prompt": system_prompt} if system_prompt else {},
                memory=None,
                chain_type="stuff",
    ...
    def process_documents_to_vectorstore(self):
        ...
        try:
            if not os.path.exists(self.airtable_records):
                raise FileNotFoundError(f"Records file not found: {self.airtable_records}")
                ...
                for line_num, line in enumerate(file, 1):
                    try:
                        ...
                        if current_words > MAX_BATCH_WORDS or current_chars > MAX_BATCH_CHARS:
                            skipped_size += 1
                            print(f"跳过过大记录 - 词数: {current_words}/{MAX_BATCH_WORDS}, 字符数: {current_chars}/{MAX_BATCH_CHARS}")
                            continue
                        
                        # 检查添加此记录是否会超出批处理限制
                        if (batch_word_count + current_words >= MAX_BATCH_WORDS or 
                            batch_char_count + current_chars >= MAX_BATCH_CHARS):
                            # 如果会超出限制，先处理当前批次
                            if batch_texts:
                                ...
                                try:
                                    ...
                                    if processed_records == 0:
                                        self.vectorstore = FAISS.from_texts(batch_texts, self.embeddings)
                                    else:
                                    ...
                                    print(f"批次处理完成: {batch_size} 条记录 | 进度: {processed_records}/{total_records} | 剩余: {remaining_records}")
                                    
                                    if processed_records >= 100 and processed_records % 100 <= len(batch_texts):
                                    ...
                                    try:
                                        ...
                                        for i in range(0, len(batch_texts), sub_batch_size):
                                            ...
                                            print(f"处理子批次 {i//sub_batch_size + 1}/{(len(batch_texts) + sub_batch_size - 1)//sub_batch_size}，大小: {len(sub_batch)}")
                                            ...
                                            try:
                                                if processed_records == 0 and i == 0:
                                                    self.vectorstore = FAISS.from_texts(sub_batch, self.embeddings)
                                                else:
                                                    self.vectorstore.add_texts(sub_batch)
                                                
                                                processed_records += len(sub_batch)
                                                print(f"子批次处理成功: {len(sub_batch)} 条记录")
            ...
            # 输出简化的统计信息
            print(f"总记录数: {total_records}, 成功处理: {processed_records}, 因大小跳过: {skipped_size}, 因JSON解析错误跳过: {json_errors}, 因API错误跳过: {api_errors}")
    ...
    def _load_system_prompt(self):
        ...
        try:
            if os.path.exists(self.prompt_file):
                ...
                print(f"Prompt file not found at {self.prompt_file}, no prompt will be used")
                return None
        except Exception as e:
            print(f"Error loading system prompts: {str(e)}, no prompt will be used")
            raise
        ...
        try:
            ...
            if chat_history:  # if chat_history is not empty    
                for message in chat_history:
                    if hasattr(message, 'user_message') and hasattr(message, 'ai_response'):
                        formatted_history.append(HumanMessage(content=str(message.user_message)))
                        formatted_history.append(AIMessage(content=str(message.ai_response)))
    ...
    def check_and_clean_vectorstore(self):
        ...
        try:
            ...
            if os.path.exists(self.store_path):
                print(f"Found existing vector store, cleaning up files in {self.store_path}...")
