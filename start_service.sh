...
if ! command -v python3 &> /dev/null; then
    ...
    if command -v apt-get &> /dev/null; then
        sudo apt-get update && sudo apt-get install -y python3 python3-pip python3-venv || { 
            echo -e "${RED}Failed to install Python3 ${NC}"; exit 1; 
        }
        ...
        echo -e "${RED}Failed to install Python3, please install it manually${NC}"
        exit 1
    fi
...
if [ ! -d "venv" ]; then
    ...
    python3 -m venv venv || { 
        echo -e "${RED}Failed to create virtual environment${NC}"; exit 1; 
    }
fi
...
cd backend || { 
    echo -e "${RED}Failed to enter backend directory${NC}"; exit 1; 
}
